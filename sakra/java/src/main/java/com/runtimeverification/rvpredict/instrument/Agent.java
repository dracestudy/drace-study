...
import com.runtimeverification.rvpredict.log.OnlineLoggingEngine;
...
public class Agent implements ClassFileTransformer, Constants {
    ...
    private static void appendSelfToSystemClassLoaderSearch(Instrumentation inst) {
        try {
            String jarPath = System.getProperty("rv.jar-path");
            ...
            throw new RuntimeException("Could not expose RV-Predict listeners to the test JVM", e);
        }
    }

    public static void premain(String agentArgs, Instrumentation inst) {
        instrumentation = inst;
        appendSelfToSystemClassLoaderSearch(inst);
        ...
        if (config.isProfiling()) {
            ...
            if (config.isWindowed()) {
                ...
                loggingEngine = new OnlineLoggingEngine(config, RVPredictRuntime.metadata);
            }
        }
